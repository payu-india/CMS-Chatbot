...
When a transaction is successful, the response format is as follows:
...
### Transaction with Refund Exists

When a refund exists for the transaction, the response format is as follows:
...
### Transaction with No Refund (Auto Refund Disabled)

When no refund exists for a transaction (auto refund disabled), the response format is as follows:
