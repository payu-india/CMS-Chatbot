---
title: Absolute Split - PayU Hosted Integration
deprecated: false
hidden: true
metadata:
  robots: index
---
---
title: Absolute Split During Transaction - PayU Hosted Checkout
excerpt: >-
  Split a payment by fixed amount among child merchants at transaction time
  using the `_payment` API with PayU Hosted Checkout.
deprecated: false
hidden: false
metadata:
  title: Absolute Split During Transaction - PayU Hosted Checkout | PayU
  description: >-
    API reference for absolute (amount-based) split settlements during a PayU
    Hosted Checkout `_payment` request. Includes splitRequest JSON, hash
    calculation, and callback response parameters.
  keywords:
    - split settlements
    - absolute split
...
Use this API to collect a payment on PayU Hosted Checkout and split the transaction amount by fixed value among child merchants at transaction time. Post the `splitRequest` parameter with `_payment`; PayU redirects the customer to the hosted payment page and returns split details in the success or failure callback.

<Callout icon="📘" theme="info">
  **Note**: Specify two decimal places for each split amount, and ensure the sum of all `aggregatorSubAmt` values equals the transaction **amount**.
</Callout>
...
| amount `mandatory`       | `String` Total payment amount for the transaction. Must equal the sum of all split amounts.                                                                                      | 2000                                                                                       |
...
| splitRequest `mandatory` | `JSON` Split payment details in JSON format. URL-encode this value when posting as form data. For field descriptions, see [splitRequest JSON fields](#splitrequest-json-fields). | `{"type":"absolute","splitInfo":{...}}`                                                    |
...
| hash `mandatory`         | `String` SHA-512 hash calculated on your server. When `splitRequest` is included, append it at the end of the hash string. See [Hash calculation](#hash-calculation).            | dc7e3ea73aa5a95c9a90e8732952f980...                                                        |
...
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- |
| type `mandatory`                                        | `String` Split type. Use **absolute** for amount-based splits. The amount for each child is set in `aggregatorSubAmt`.                | absolute                                        |
...
| splitInfo.\<merchantKey>.aggregatorSubTxnId `mandatory` | `String` Unique sub-transaction ID for the child merchant, generated by the merchant.                                                 | child_1779181092078_285                         |
| splitInfo.\<merchantKey>.aggregatorSubAmt `mandatory`   | `String` Fixed amount allocated to this child. Use two decimal places. Sum across all children must equal the transaction **amount**. | 1000                                            |
| splitInfo.\<merchantKey>.aggregatorCharges `optional`   | `String` Fixed amount allocated to aggregator charges. Only parent aggregators can include this field.                                | 0.00                                            |
...
<Accordion title="Sample Request" icon="fa-code">
  ...
  curl -X POST "https://test.payu.in/_payment" \
    ...
    -d "hash=dc7e3ea73aa5a95c9a90e8732952f980189a2e3f610f841a7b4868f5af4e04edb8790a6679db6e864b188dd69c0e49f41640ecafcb1988ad7e1c5f42a37e0547"
  ```
  ...
  split_request = (
      '{"type":"absolute","splitInfo":{'
      '"gYoEaY":{"aggregatorSubTxnId":"child_1779181092078_285",'
      '"aggregatorSubAmt":"1000","aggregatorCharges":"0.00"},'
      '"5rgA73":{"aggregatorSubTxnId":"child_1779181092078_6955",'
      '"aggregatorSubAmt":"1000","aggregatorCharges":"0.00"}}}'
  )
  ...
  data = {
      ...
      "hash": "dc7e3ea73aa5a95c9a90e8732952f980189a2e3f610f841a7b4868f5af4e04edb8790a6679db6e864b188dd69c0e49f41640ecafcb1988ad7e1c5f42a37e0547",
  }
...
<Accordion title="Sample Response" icon="fa-code">
  ...
  &splitInfo={"splitStatus":"success","splitSegments":[{"merchantKey":"gYoEaY","amount":1000,"subvention_amount":0,"txnId":"child_1779181092078_285"},{"merchantKey":"5rgA73","amount":1000,"subvention_amount":0,"txnId":"child_1779181092078_6955"}]}
  ```
...
| Split validation error | Split amounts do not match transaction amount | Sum of `aggregatorSubAmt` values does not equal `amount`. | Adjust split amounts so the total equals the transaction amount.          |
...
* [Split by Percentage During Transaction - PayU Hosted Checkout](ref:split-by-percentage-during-transaction-payu-hosted-checkout) — Split by percentage on PayU Hosted Checkout.
* [Absolute Split During Transaction](ref:absolute-split-during-transaction) — Same split type for server-to-server (card) `_payment` flows.
