...
| amount          | This field contains the requested transaction amount echoed back in the payment response.                                                                                         |
| authpayuid      | Unique PayU transaction ID for the original registration/mandate. Used to link recurring transactions to the original mandate.                                                    |
| user_credentials| Credentials used for the transaction, if any. Typically used for authentication or identifying the user in PayU systems.                                                          |
| card_token      | Tokenized representation of the card used for the transaction, if applicable. May be blank for UPI or non-card transactions.                                                      |
| payuid          | PayU’s transaction ID for the processed recurring transaction. Merchant can use this field for reference in the settlement report.                                                |
| status          | Status of the transaction (e.g., "captured" for successful transactions).                                                                                                         |
| field9          | Description of transaction status, which can help the merchant in providing better customer communication.                                                                        |
