...
<Accordion title="Get Downpayment+EMI list" icon="fa-reply">
Use the following to get the Downpayment and EMI list of offers for the customer.
```curl
curl -X POST "https://api.payu.in/fems/v1/checkout/detail"
-H "accept: application/json"
-H "Content-Type: application/json"
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        {
            requestId = "413088215",
            transactionDetails = new { amount = 15001.0 },
            customerDetails = new { mobile = "9910522063" },
            filters = new
            {
                paymentOptions = new
                {
                    emi = new { cardless = "all" },
                    bnpl = "all"
                }
            },
            useCase = new
        ...
        try
        {
            HttpResponseMessage response = await client.PostAsync("https://api.payu.in/fems/v1/checkout/detail", content);
...
getCheckoutDetail().then(result => console.log("Done:", result)).catch(console.error);
```
...
public class CheckoutDetail {
    public static void main(String[] args) {
        ...
        String jsonPayload = "{"
            + ""requestId": "413088215","
            + ""transactionDetails": { "amount": 15001.0 },"
            + ""customerDetails": { "mobile": "9910522063" },"
            + ""filters": {"
            + "    "paymentOptions": {"
            + "        "emi": { "cardless": "all" },"
            + "        "bnpl": "all""
        ...
        HttpRequest request = HttpRequest.newBuilder()
            ...
            .header("accept", "application/json")
            .header("Content-Type", "application/json")
            .header("Date", "Fri, 24 Jul 2026 05:51:20 GMT")
            .header("Authorization", "hmac username="YOUR_MERCHANT_KEY", algorithm="sha512", headers="date", signature="GENERATED_SIGNATURE"")
