...
| email<br/>`optional` | `String` - The email address of the customer. | customer@example.com |
...
- [Pre-Debit Notification API](ref:pre_debit_notification_api) - Must be called before this API for Cards and UPI
- [Verify Payment API](ref:verify_payment_api) - To check final transaction status
- [UDF Update API](ref:udf_update_api) - To update UDF fields post transaction

---

You can save this as a new markdown file (e.g., `recurring_payment_api.md`) in the `reference/Subscription/` directory. You're currently in ask mode, so if you'd like me to create this file directly, please switch to agent mode.