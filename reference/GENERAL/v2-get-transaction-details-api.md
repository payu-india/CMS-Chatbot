...
<Table align={["left","left","left"]}>
  ...
  <tbody>
    <tr>
      ...
      <td>
        The current date and time in GMT. For example, if the current time in India is 18:00:00 IST, the time in the Date header should be 12:30:00 GMT.
      </td>

      <td>
        `Tue, 12 Aug 2025 16:44:30 GMT`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        Base64-encoded SHA-256 hash of the JSON request body posted to the server.
      </td>

      <td>
        `MFJ1ms99nEgMzaC9apz0eTUSnL7JSUYtHNb+w1lwRic=`
      </td>
    </tr>
    ...
    <tr>
      ...
      <td>
        HMAC authorization header in the following format:

        `hmac username="<merchant_key>", algorithm="hmac-sha256", headers="date digest", signature="<signature>"`

        Where:

        * **username**: Your merchant key provided by PayU.
        * **algorithm**: Must be `hmac-sha256`.
        * **headers**: Must be `date digest`.
        * **signature**: HMAC-SHA256 of the signing string using your merchant salt as the secret. The signing string is in the format `date: <Date>\ndigest: <Digest>`, where `<Date>` and `<Digest>` are the same values sent in the request headers.
      </td>
