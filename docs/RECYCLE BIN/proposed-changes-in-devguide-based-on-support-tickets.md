...
| # | Title | Severity | Fix Type | Pages Affected |
|---|---|---|---|---|
| 4 | Cordova SDK — `surl` validation stricter than web | 🟠 High | New callout note | 2 |
| 5 | React Native SDK — `generateHash` error message is misleading | 🟠 High | New callout + correction | 2 |
| 6 | UPI Intent/Collect overlap by device type | 🟠 High | New section | 3 |
| 7 | `GetUserCards` API — saved cards not returned for same credentials | 🟠 High | New troubleshooting section | 2 |
| 8 | CheckoutPro — disabled payment modes re-appear after timeout | 🟠 High | New callout note | 3 |
| 9 | `merchant_key_not_register_for_phonepe` error undocumented | 🟠 High | New entry in troubleshooting table | 2 |
| 10 | `intentURIData` returns a single URL, not per-app URLs | 🟠 High | Clarification to existing content | 2 |
| 11 | Salt v2 migration undocumented | 🟠 High | New section on existing pages | 5 |
| 13 | SI `billingCycle: ADHOC` not in parameter table | 🟡 Medium | New row in parameter table | 2 (3rd page already correct) |
| 14 | Webhook UI — delete and duplicate event creation | 🟡 Medium | Correction to existing content + new note | 3 |
| 15 | "Convenience Fee" vs "Internet Handling Charges (IHC)" | 🟡 Medium | New terminology note | 5 |
| 16 | UPI QR test transactions failing with undocumented error | 🟡 Medium | New troubleshooting note | 2 |

## Cordova SDK — `surl` validation stricter than web

**Severity:** 🟠 High  
**Fix type:** New callout note

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| Steps to Integrate (Cordova CheckoutPro SDK) | https://docs.payu.in/docs/cordova-checkoutprosdk-integration-steps | `Step 2: Set up Callback` |
| Cordova CheckoutPro SDK (overview) | https://docs.payu.in/docs/cordova-sdk-introduction | First section / Prerequisites |

### Where exactly to insert

**Page:** Steps to Integrate (Cordova CheckoutPro SDK)  
**Heading:** `Step 2: Set up Callback`  
**Position:** Before the `responseCallBack` code block

**Page:** Cordova CheckoutPro SDK (overview)  
**Heading:** First section / Prerequisites  
**Position:** At the top of the page, before "Step 1: SDK Integration"
...
**Severity:** 🟠 High  
**Fix type:** New callout + correction

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| Android Integration (React Native) | https://docs.payu.in/docs/reactnative-checkoutpro-android-integration | `generateHash` section (under the React Native 0.82.0+ notice) |
| Generate Dynamic Hash (React Native) | https://docs.payu.in/docs/generate-dynamic-hash-react | `Procedure` |

### Where exactly to insert

**Page:** Android Integration (React Native)  
**Heading:** `generateHash`  
**Position:** After the `generateHash` code block (after the `PayUBizSdk.hashGenerated(result)` example)

**Page:** Generate Dynamic Hash (React Native)  
**Heading:** `Procedure`  
**Position:** After the last paragraph of the Procedure section ("The server will give that hash back to your app...")
...
**Severity:** 🟠 High  
**Fix type:** New section

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| UPI Integration (Merchant Hosted) | https://docs.payu.in/docs/collect-payments-with-upi-seamless | After the `⚠️ Important UPI Integration Changes` callout |
| UPI Intent with S2S Integration | https://docs.payu.in/docs/upi-intent-server-to-server | `Step 1: Fetch the List of UPI and Smart Intent Supported Apps` |
| UPI Collect Disablement Information | https://docs.payu.in/docs/upi-collect-disablement-information | `Exemptions` |

### Where exactly to insert

**Page:** UPI Integration (Merchant Hosted)  
**Heading:** After the `⚠️ Important UPI Integration Changes` callout at the top  
**Position:** Add a new subsection immediately after the existing NPCI mandate callout

**Page:** UPI Intent with S2S Integration  
**Heading:** `Step 1: Fetch the List of UPI and Smart Intent Supported Apps`  
**Position:** After the first paragraph of Step 1

**Page:** UPI Collect Disablement Information  
**Heading:** `Exemptions`  
**Position:** After the exemptions table
...
**Severity:** 🟠 High  
**Fix type:** New troubleshooting section

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| Get User Cards API - Model 3 | https://docs.payu.in/reference/get_user_cards_api_model3 | After `Sample response` section (end of page) |
| Collect Payments using a Tokenized Card | https://docs.payu.in/docs/collect-payments-using-a-saved-card | `Get the tokenized card details` |

### Where exactly to insert

**Page:** Get User Cards API - Model 3  
**Heading:** `Sample response`  
**Position:** After the Sample response section — add a new `## Troubleshooting` section at the end of the page

**Page:** Collect Payments using a Tokenized Card  
**Heading:** `Get the tokenized card details`  
**Position:** After the description of `get_user_details` API
...
| Page Title | URL | Section/Heading |
|---|---|---|
| Customise Your Integration (Android CheckoutPro) | https://docs.payu.in/docs/android-checkoutpro-custom-integrations | Payment mode customisation / `payUCheckoutProConfig` section |
| Advanced Integration (iOS CheckoutPro) | https://docs.payu.in/docs/ios-checkoutprosdk-advanced-integration | Customising payment options / removing payment modes section |
| Android Integration (React Native) | https://docs.payu.in/docs/reactnative-checkoutpro-android-integration | Payment options customisation section |

### Where exactly to insert

**Page:** Customise Your Integration (Android CheckoutPro)  
**Heading:** `payUCheckoutProConfig` (payment mode customisation section)  
**Position:** After the code block showing how to disable payment modes using `payUCheckoutProConfig`

**Page:** Advanced Integration (iOS CheckoutPro)  
**Heading:** Customising payment options / removing payment modes  
**Position:** After the code block showing disabled payment modes

**Page:** Android Integration (React Native)  
**Heading:** Payment options customisation section  
**Position:** After the relevant config code block
...
*Note: Adapt the callout for Swift/Objective-C syntax context on the iOS page.*
...
**Severity:** 🟠 High  
**Fix type:** New entry in troubleshooting table

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| Troubleshooting CheckoutPro SDK (Android) | https://docs.payu.in/docs/android-checkoutpro-troubleshoot-errors | Existing error table / troubleshooting list |
| FAQs - Android SDK | https://docs.payu.in/docs/faqs-android-sdk | FAQ section about errors/troubleshooting |

### Where exactly to insert

**Page:** Troubleshooting CheckoutPro SDK (Android)  
**Heading:** Existing error/troubleshooting table  
**Position:** Add as a new row in the existing error table

**Page:** FAQs - Android SDK  
**Heading:** FAQ section (errors/troubleshooting)  
**Position:** Add as a new FAQ entry in the troubleshooting section
...
**Severity:** 🟠 High  
**Fix type:** Clarification to existing content

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| UPI Intent with S2S Integration | https://docs.payu.in/docs/upi-intent-server-to-server | `Step 2: Invoke UPI Intent on customer's device` |
| UPI Intent - Non SDK Flow | https://docs.payu.in/docs/upi-smart-intent-non-sdk-flow | `Step 2: Get Intent URI` |

### Where exactly to insert

**Page:** UPI Intent with S2S Integration  
**Heading:** `Step 2: Invoke UPI Intent on customer's device`  
**Position:** After the paragraph "Using the IntentURIData value in response" and before the sample URL

**Page:** UPI Intent - Non SDK Flow  
**Heading:** `Step 2: Get Intent URI`  
**Position:** After the step description, before the request parameters table
...
**Severity:** 🟠 High  
**Fix type:** New section on existing pages

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| Access Production Key and Salt | https://docs.payu.in/docs/generate-merchant-key-and-salt-on-payu-dashboard | Salt section (currently shows `Salt-32 bit`) |
| Access Test Merchant Key and Salt | https://docs.payu.in/docs/generate-test-merchant-key-and-salt | Section describing Salt-32 bit |
| Generate Hash (Request/Response) | https://docs.payu.in/docs/hashing-request-and-response | `🚧 Salt Security` callout |
| Generate Hash (PayU Hosted) | https://docs.payu.in/docs/generate-hash-payu-hosted | `🚧 Salt Security` callout |
| Generate Hash (Merchant Hosted) | https://docs.payu.in/docs/generate-hash-merchant-hosted | `🚧 Salt Security` callout |

### Where exactly to insert

**Page:** Access Production Key and Salt  
**Heading:** Salt section (currently shows `Salt-32 bit`)  
**Position:** After the existing salt description — add as a new subsection

**Page:** Access Test Merchant Key and Salt  
**Heading:** Section describing Salt-32 bit  
**Position:** After the note about copying the Salt

**Pages:** Generate Hash (all three hash pages)  
**Heading:** `🚧 Salt Security` callout  
**Position:** After the existing callout
...
## Cross-Border on-hold — two conflicting update APIs
...
| Page Title | URL | Section/Heading |
|---|---|---|
| On-Hold Settlements - Cross-Border Payments | https://docs.payu.in/docs/on-hold-settlements-cross-border-payments | `Types of Settlement Holds` table → `Provide valid invoice number` row |
| On-Hold and Settlement APIs - Cross-Border Payments | https://docs.payu.in/docs/on-hold-and-settlement-apis-cross-border-payments | `Update On-Hold Transactions API` |

### Where exactly to insert

**Page:** On-Hold Settlements - Cross-Border Payments  
**Heading:** `Types of Settlement Holds`  
**Position:** Directly after the settlement holds table (the row referencing multiple update methods)

**Page:** On-Hold and Settlement APIs - Cross-Border Payments  
**Heading:** `Update On-Hold Transactions API`  
**Position:** After the description of the Update On-Hold Transactions API

### What to add or change

**On On-Hold Settlements - Cross-Border Payments (after the table):**

```
> 📌 **Which API to use for updating on-hold transactions?**
> There are two APIs referenced in the documentation. Here is when to use each:
>
> | API | Use when |
> |---|---|
> | **UDF Update API** (`/merchant/postservice.php?form=2` with `command=update_payment_transaction`) | Updating UDF fields (invoice number via udf5, PAN via udf1, DOB via udf3) for a transaction you already processed via `_payment` API |
> | **Update On-Hold Transaction Details API** (`/opgsp/updateOnHoldTxnDetails`) | Updating buyer details, PAN, DOB, and invoice information for cross-border on-hold transactions via the dedicated CB settlement API |
>
> For standard cross-border on-hold resolution, use the **UDF Update API** unless your integration team has specifically been instructed to use `updateOnHoldTxnDetails`.
```

**On On-Hold and Settlement APIs - Cross-Border Payments (after Update On-Hold Transactions API description):**

```
> 📌 If you have already integrated the UDF Update API (`update_payment_transaction`), that API achieves the same outcome for most on-hold scenarios. The Update On-Hold Transactions API is the recommended path for new integrations using the Cross-Border v2 API stack.
```

---

## Inaccuracy 13: SI `billingCycle: ADHOC` not in parameter table

**Severity:** 🟡 Medium  
**Fix type:** New row in parameter table

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| Standing Instruction Parameter Details (Android) | https://docs.payu.in/docs/android-standing-instruction-parameters | `Step 2: Post Parameters` → `billingCycle` row |
| PayU Standing Instructions Parameters (iOS) | https://docs.payu.in/docs/ios-standing-instructions-parameters | `Step 2: Post parameters` → `billingCycle` field |
| [S2S] UPI Consent Transaction - Cross Border | https://docs.payu.in/docs/upi-consent-transaction-cb | `si_details JSON Object` → `billingCycle` field (already includes ADHOC — no change needed) |

### Where exactly to insert

**Page:** Standing Instruction Parameter Details (Android)  
**Heading:** `Step 2: Post Parameters`  
**Position:** Within the `billingCycle` parameter row — extend the existing description

**Page:** PayU Standing Instructions Parameters (iOS)  
**Heading:** `Step 2: Post parameters`  
**Position:** After the existing billingCycle description
...
*Adapt code/syntax examples for Swift context on the iOS page.*
...
**Severity:** 🟡 Medium  
**Fix type:** Correction to existing content + new note

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| Create a New Webhook | https://docs.payu.in/docs/create-a-new-webhook | Step-by-step webhook creation guide (end of page / Step 4) |
| Delete a Webhook | https://docs.payu.in/docs/delete-a-webhook-on-dashboard | Step-by-step delete guide |
| Manage Webhooks using Dashboard | https://docs.payu.in/docs/manage-webhooks-using-dashboard | Overview section |

### Where exactly to insert

**Page:** Create a New Webhook  
**Heading:** Step-by-step webhook creation guide  
**Position:** After the "Click Create to create a webhook" step

**Page:** Delete a Webhook  
**Heading:** Step-by-step delete guide  
**Position:** Before or after the existing delete instructions

**Page:** Manage Webhooks using Dashboard  
**Heading:** Overview section  
**Position:** After the intro paragraph
...
**Severity:** 🟡 Medium  
**Fix type:** New terminology note

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| Collect Additional Charges | https://docs.payu.in/docs/collect-additional-charges | First paragraph / introduction |
| Setup Convenience Fee (Android CheckoutPro) | https://docs.payu.in/docs/android-checkoutpro-setupconveniencefee | Page title / first section |
| Setup Convenience Fee (iOS CheckoutPro) | https://docs.payu.in/docs/ios-checkoutpro-setupconveniencefee | Page title / first section |
| Setup Convenience Fee (Flutter) | https://docs.payu.in/docs/setup-convenience-fee-1 | Page title / first section |
| UPI CC & CL Integration with Convenience Fee | https://docs.payu.in/docs/upi-cc-cl-integration-with-convenience-fee | `Key Highlights` |

### Where exactly to insert

**Pages:** Collect Additional Charges; Setup Convenience Fee (Android, iOS, Flutter)  
**Heading:** Page title / first section  
**Position:** At the very top of the page, before the existing content

**Page:** UPI CC & CL Integration with Convenience Fee  
**Heading:** `Key Highlights`  
**Position:** Before the Key Highlights section
...
**Severity:** 🟡 Medium  
**Fix type:** New troubleshooting note

### Affected Page(s)

| Page Title | URL | Section/Heading |
|---|---|---|
| Integrate UPI QR | https://docs.payu.in/docs/integrate-upi-qr | `Test the Integration` / `Troubleshooting` section (or end of page if absent) |
| APIs for UPI QR Integration | https://docs.payu.in/docs/apis-for-upi-qr-integration | API parameters section / API description |

### Where exactly to insert

**Page:** Integrate UPI QR  
**Heading:** `Test the Integration` or `Troubleshooting` (if present); otherwise end of page  
**Position:** End of page, or within an existing testing/troubleshooting section

**Page:** APIs for UPI QR Integration  
**Heading:** API parameters section  
**Position:** After the description of required parameters
