...
2. Email **integration@payu.in** with:
...
| `callback_on_failure` | Sends real-time webhook notifications for pending and failed payment states, not just successful payments. By default, webhooks are only sent on success. | Email support@payu.in or contact KAM | UAT + Prod (separate) |
...
Log in to the **PayU Dashboard** → **Settings** → **Payment Methods**. This shows which payment instruments (card, UPI, net banking, wallets, etc.) are enabled for your checkout.

### S2S flags and merchant config

There is no self-serve view for S2S flags (`txn_s2s_flow`, `disable_all_retry`, etc.) or most merchant config settings. Contact your KAM to confirm which flags are currently active on your MID.

### Offers

Dashboard → **Offers** — if the Offers Engine is enabled, you will see the option to create and manage offers.
...
1. Request UAT activation to develop and test your integration.
...
- Dynamic QR (DBQR) Integration Guide
- Auth & Capture — Pre-Authorization Integration Guide
- Standing Instructions (SI) — Overview
- eNACH Bulk Debit Trigger — Dashboard Upload Guide
