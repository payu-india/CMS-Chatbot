---
title: Merchant Hosted Checkout - Link to Individual Integrations
deprecated: false
hidden: true
metadata:
  robots: index
...
### Before you begin
  <br />
**Get started with your eCommerce page**   
Ensure the you have set up your eCommerce page to collect the details according to the payment modes with which you wish to integration. 
  <br />
> Example For Cards integration, you require the following fields on your eCommerce page:
>  * Card number
>  * Expiry Date (MM/YY)
>  * CVV
>  * Name on card
  <br />
Validating the inputs based on payment modes you are integrating to ensure the following:
  ...
  * Check the eligibility of your customer's card.
  ...
  Select any of the following the card to refer to the API that you have to use for validating the input based on the payment mode.
...
### Payment mode integration
  ...
  **Sample request**
  The following sample request is for collecting payment using credit card, where the pg=CC and bankcode=CC.
...
<Accordion title="Step 1.3: Generate Hash" icon="fa-key">
  ```
  json|txnid|amount|productinfo|firstname|email|udf1|udf2|udf3|udf4|udf5|udf6|udf7|udf8|udf9|udf10|SALT
  ```
  ...
  **PHP Response Verification:**
  ...
  // Reverse hashing logic: Concatenate fields in the exact order specified by PayU
  ...
  // Use hash_equals to prevent timing attacks during hash comparison
