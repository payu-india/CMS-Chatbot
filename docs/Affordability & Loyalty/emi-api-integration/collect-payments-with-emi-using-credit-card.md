...
> * For Server-to-Server integration, CC-EMI works on **txn\_s2s\_flow=1, 2,** or **4**, whereas, DC-EMI only works on **txn\_s2s\_flow=1**. The same **base64Decoder** logic will be used to decode the encrypted **acsTemplate** (in case of txn\_s2s\_flow=4) and post\_data (in case of txn\_s2s\_flow=1 or 2).
