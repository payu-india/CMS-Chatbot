...
<Accordion title="Step 1: Fetch Balance API" icon="fa-wallet">
 ...
 The **Fetch Balance** API retrieves the current TWID points balance for a customer using their mobile number or user identifier. 
  ...
  <Accordion title="Request Parameters" icon="fa-table">
    ...
    | ----------- | ------ | -------- | ----------------------------------------------- |
    | `mobile`    | String | Yes      | Customer's registered mobile number (10 digits) |
    | `timestamp` | String | Yes      | Request timestamp in ISO 8601 format            |
    | `userId`    | String | Optional | Alternative customer identifier                 |
    | `email`     | String | Optional | Customer's registered email address             |
  </Accordion>
...
<Accordion title="Step 2: Enquire Transaction API (Optional)" icon="fa-search">
  ...
  <Accordion title="Purpose of API" icon="fa-info-circle">
    The Enquire Transaction API is an optional but recommended step for transaction management. It enables:

    • **Transaction status tracking** - Monitor the progress of TWID point operations
    • **Reconciliation support** - Match transactions with internal records
    • **Customer service** - Provide real-time transaction updates to customers
    • **Audit trail** - Maintain comprehensive transaction history
    • **Error investigation** - Troubleshoot failed or pending transactions
    • **Compliance reporting** - Generate transaction reports for regulatory requirements
  </Accordion>

  <Accordion title="Request Parameters" icon="fa-table">
    | Parameter        | Type    | Required | Description                                             |
    | ---------------- | ------- | -------- | ------------------------------------------------------- |
    | `transactionId`  | String  | Yes      | Unique transaction identifier from previous API calls   |
    | `timestamp`      | String  | Yes      | Request timestamp in ISO 8601 format                    |
    | `mobile`         | String  | Optional | Customer's mobile number for additional validation      |
    | `includeDetails` | Boolean | Optional | Include detailed transaction breakdown (default: false) |
  </Accordion>
...
The Hold TWID Points API creates a temporary reservation of points to secure them during checkout. 
  ...
  <Accordion title="Request Parameters" icon="fa-table">
    ...
    | ----------------------- | ------- | -------- | ------------------------------------------------------------------------ |
    | `mobile`                | String  | Yes      | Customer's registered mobile number (10 digits)                          |
    | `pointsToHold`          | Integer | Yes      | Number of TWID points to hold/reserve                                    |
    | `merchantTransactionId` | String  | Yes      | Unique transaction ID from merchant system                               |
    | `timestamp`             | String  | Yes      | Request timestamp in ISO 8601 format                                     |
    | `holdDuration`          | Integer | Optional | Hold duration in minutes (default: 15, max: 30)                          |
    | `categoryPreference`    | String  | Optional | Preferred point category to hold \["CASHBACK", "REWARDS", "PROMOTIONAL"] |
  </Accordion>
...
<Accordion title="Step 4: Redeem TWID Points API" icon="fa-gift">
  ...
  <Accordion title="Request Parameters" icon="fa-table">
    | Parameter                | Type    | Required | Description                                |
    | ------------------------ | ------- | -------- | ------------------------------------------ |
    | `holdId`                 | String  | Yes      | Hold ID received from Hold TWID Points API |
    | `merchantTransactionId`  | String  | Yes      | Unique transaction ID from merchant system |
    | `timestamp`              | String  | Yes      | Request timestamp in ISO 8601 format       |
    | `actualRedemptionAmount` | Integer | Optional | Actual points to redeem (≤ held points)    |
    | `orderDetails`           | Object  | Optional | Order information for receipt and records  |
  </Accordion>
