...
1. Implement any of the following split APIs to split at your end using the transaction id generated:
   * [Split After Transaction API](ref:split_after_transaction_api) under API Reference
   * [Split During Transaction using \_payment](ref:split-during-transaction-using-_payment) under API Reference
...
2. [Get Aggregator/Parent Transaction Info](ref:get_aggregator_parent_transaction_info_api)  Get the split information of the parent transaction in the Aggregator flow.
3. [Release Settlement API](ref:release_settlement_api): Implement the release payment API.
