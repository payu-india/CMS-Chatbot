...
* **How is PayU supporting the split payments? Does it support split by percentage?**
  ...
  * [Split During Payment](ref:split-during-transaction-using-_payment)
...
* **I am getting the following error message while using release_settllement API. What is the reason for this error?**
...
* **Can we make multiple split calls for Settlement APIs?**
  ...
  Settlement API (**release_settlement** API) can be called individually for each split or can be called for multiple splits together. For more information on the **release_settlement** API, refer to [Release Settlement API](ref:release_settlement_api).
...
* **Where do I include the split information in the cancel_refund_transaction API when posting the cancellation of a transaction?**
