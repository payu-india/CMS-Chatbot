---
title: One-Time Mandate Experience
excerpt: ''
deprecated: false
hidden: true
metadata:
...
The One-Time Mandate supports a variety of payment flows, including eNACH, UPI Autopay, and Standing Instructions (SI) on Cards, all accessible through the PayU Checkout page. It works with PayU Hosted Checkout integration, ensuring a smooth and secure transaction experience.

> 📘 Note:
>
> The One-Time Mandate is supported only with PayU Hosted Checkout integration. For the request and response parameters, refer to [One-Time Mandate Consent Transaction using PayU Hosted Checkout](ref:one-time-mandate-consent-transaction).

Currently, the standard transaction flow for one-time transactions, integrated with a PayU Recommendation Engine and SI capabilities.
...
3. User will enter the OTP required for thetransaction.
