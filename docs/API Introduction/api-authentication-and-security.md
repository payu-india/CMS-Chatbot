...
excerpt: >-
  Learn how PayU authenticates API requests using merchant key and salt hashing,
  OAuth access tokens, and HMAC headers — and how to secure every request.
deprecated: false
hidden: false
metadata:
  title: PayU API Authentication and Security
  description: >-
    Authenticate PayU API requests with merchant key, salt, SHA-512 hash, OAuth
    tokens, or HMAC headers. Includes hash formulas for Payment, General, SI,
    TPV, and Split Settlements APIs.
  keywords:
