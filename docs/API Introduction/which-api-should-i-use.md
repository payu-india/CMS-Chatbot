...
excerpt: >-
  Decision guide to help developers choose the right PayU API for payments,
  refunds, subscriptions, payouts, partners, and marketplace flows.
deprecated: false
hidden: false
metadata:
  title: Which PayU API Should I Use? | API Decision Guide
  description: >-
    Choose the right PayU API for your use case — collect payments, verify
    transactions, refunds, subscriptions, payouts, partner onboarding, and
    split settlements.
  keywords:
    - which PayU API
    - PayU API decision guide
    - PayU payment API choice
    - PayU collect payment API
    - PayU payouts API
    - PayU subscription API
  robots: index
next:
  description: ''
---
Choosing the right PayU API depends on what you want to accomplish — not on how PayU organizes products internally. Use this decision guide to map your developer workflow to the correct API family, then open the matching Integration Guide and API Reference.
