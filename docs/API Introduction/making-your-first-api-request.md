...
excerpt: >-
  Step-by-step workflow to authenticate with PayU, call a Test environment API,
  create a payment, and verify the transaction status.
deprecated: false
hidden: false
metadata:
  title: Make Your First PayU API Request
  description: >-
    Step-by-step guide to make your first PayU API request — get Test
    credentials, generate a hash, call Collect Payment or Verify Payment, and
    confirm the result.
  keywords:
    - first PayU API request
    - PayU API quickstart
    - PayU test API call
    - PayU collect payment example
    - PayU verify payment example
  robots: index
next:
  description: ''
---
This guide walks through the shortest path from zero to a successful PayU API call in **Test**.
