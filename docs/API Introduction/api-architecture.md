...
excerpt: >-
  Learn how PayU APIs are organized across Collect Payment, General APIs, OAuth
  products, and v2 endpoints so you can navigate the ecosystem confidently.
deprecated: false
hidden: false
metadata:
  title: PayU API Architecture | How PayU APIs Are Organized
  description: >-
    Understand PayU API architecture — Collect Payment (_payment), General
    command-based APIs, OAuth products, v2 payments, and how reference docs map
    to each family.
  keywords:
    - PayU API architecture
    - PayU API organization
    - PayU _payment API
    - PayU General APIs
    - PayU REST API structure
  robots: index
next:
  description: ''
---
...
| **Command-based General APIs** | Server-to-server operations using `command` + `var1…var15` | Verify Payment, Refund, BIN Info, transaction details |
| **OAuth resource APIs** | Bearer/OAuth token against product hosts | Payouts, Partner onboarding |
| **Product-specific REST APIs** | Dedicated hosts and schemas per product | BBPS, Chargeback, Zion, Cross-border, v2 payments |

```
Your application
    │
    ├─► Collect Payment (_payment) ──► create transaction / redirect / S2S auth
    ├─► General APIs (postservice) ──► verify / refund / enquire / utility
    ├─► OAuth products ──► token ──► payouts / partner resources
    └─► Product APIs ──► BBPS, Zion, Chargeback, Cross-border, POS, etc.
```
...
## How Guides, API Introduction, and Reference fit together

| Layer | Purpose | Example |
| :---- | :------ | :------ |
| **Getting Started / Collect Payments / Offerings** | Product integration journeys | Hosted Checkout guide |
| **API Introduction** | Shared API concepts across products | Auth, environments, first request |
| **API Reference** | Exact endpoints, parameters, responses, Try It | Verify Payment operation page |
