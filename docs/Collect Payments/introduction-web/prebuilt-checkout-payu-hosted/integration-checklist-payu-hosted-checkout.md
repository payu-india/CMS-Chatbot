...
<Callout icon="📘" theme="info">
  **Reference**: For more information on collecting and submitting the request parameter, refer to the following:
...
4. Confirmed the transaction status on the Server-side, if the callback fail. Use Webhooks for hearing callbacks. For more information, refer to ~~Verify Payment Status ~~ (under API Reference) and [Webhooks](doc:webhooks).
