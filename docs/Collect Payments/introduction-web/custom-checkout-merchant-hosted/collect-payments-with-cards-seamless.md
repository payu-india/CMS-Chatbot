...
| <Glossary>furl</Glossary> `mandatory` | `String` The failure URL which PayU will redirect to if the transaction fails. | https://apiplayground-response.herokuapp.com/ |
| <Glossary>surl</Glossary> `mandatory` | `String` The success URL which PayU will redirect to if the transaction is successful. | https://apiplayground-response.herokuapp.com/ |
| <Glossary>hash</Glossary> `mandatory` | `String` The <Glossary>hash</Glossary> calculated by the merchant using the <Glossary>key</Glossary> and <Glossary>Salt</Glossary>. Hash calculation logic: <Glossary>SHA-512</Glossary>(key|txnid|amount|productinfo|firstname|email|udf1|udf2|udf3|udf4|udf5||||||<Glossary>Salt</Glossary>) | calculated_hash_value |


<Accordion title=“Optional Parameters" icon="fa-table">

| Parameter | Description | Example |
|:--|:--|:--|
| address1 `optional` | `String` The first line of the billing address. *For <Glossary>Fraud Detection</Glossary>*: This information is helpful for fraud detection and chargebacks. | 123 Main Street |
