...
| Pattern | Likely cause | Recommended handling |
| --- | --- | --- |
...
| PayU signal | Customer-safe message |
| --- | --- |
| Issuer decline | Your bank declined this payment. Try another payment method or contact your bank. |
| Authentication failure | Authentication failed. Check the OTP/CVV and try again. |
| Timeout | We could not confirm the payment yet. Please wait while we verify the status. |
| Card not permitted | This card is not enabled for this transaction. Try another card or payment method. |
