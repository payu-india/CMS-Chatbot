...
Collect the response in the  <a href="https://docs.payu.in/v2/reference/_payment_s2s_upi_collection" target="_blank"> UPI Collection</a> under API Reference. The response for the S2S payment request is not similar to Merchant Hosted or PayU Hosted Checkout. For description of response parameters, refer to <a href="addl_info-payment-apis#response-for-initial-server-to-server-request" target="_blank"> Additional Info for Payment APIs</a>.
...
[block:parameters]
{
  "data": {
    ...
    "1-0": "txnS2sFlow  \n `mandatory for S2S`",
    "1-1": "`String` Indicates the transaction S2S flow type and must be set to **4** for Classic Integration.",
