...
| Parameter | Description | Example |
|:--|:--|:--|
| key<br/>`mandatory` | `String` - Field key name (from keyMappingList) | first_name |
...
1. **Poll Regularly**: Set up a scheduled job to poll the GET API daily or weekly to identify new on-hold transactions

2. **Validate Before Submitting**: Use the `validationRegex` from `keyMappingList` to validate field values before submitting

3. **Track Due Dates**: Monitor `dueDate` and prioritize transactions nearing expiry

4. **Handle Multiple Transactions**: The POST API supports batch updates - submit multiple transactions in a single request

5. **Error Handling**: Always check the `status` field in responses and implement retry logic for transient failures
