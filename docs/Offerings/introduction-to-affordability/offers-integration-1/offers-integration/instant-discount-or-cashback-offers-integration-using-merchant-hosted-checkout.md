...
| :--- | :--- | :--- |
| <Glossary>key</Glossary> | `String` Merchant key provided by PayU during onboarding. | vqpS7W |
| <Glossary>txnid</Glossary> | `String` Transaction ID, unique reference number generated by the merchant. | Txn_050220260127 |
| amount | `String` The payment amount for the transaction. | 500 |
| <Glossary>productinfo</Glossary> | `String` A brief description of the product or service. | iPhone |
| firstname | `String` The first name of the customer. | Test User |
| email | `String` The email address of the customer. | test@gmail.com |
| phone | `String` The phone number of the customer. | 9876543210 |
| <Glossary>surl</Glossary> | `String` The success URL, which is the page PayU will redirect to if the transaction is successful. | https://test.payu.in/admin/test_response |
| <Glossary>furl</Glossary> | `String` The failure URL, which is the page PayU will redirect to if the transaction fails. | https://test.payu.in/admin/test_response |
| <Glossary>hash</Glossary> | `String` Hash calculated for transaction security. For offers, use: sha512(key\|txnid\|amount\|productinfo\|firstname\|email\|udf1\|udf2\|udf3\|udf4\|udf5\|udf6\|udf7\|udf8\|udf9\|udf10\|<Glossary>offer_key</Glossary>\|offer_auto_apply\|<Glossary>Salt</Glossary>). If any key is null/not configured, the \| character must still be concatenated. | 3f1b738fe8485e21... |
| <Glossary>pg</Glossary> | `String` Payment type selector. For more info, refer to [Bank and Card Codes for Integration](https://docs.payu.in/docs/bank-and-card-codes-for-integration). | CC |
| <Glossary>bankcode</Glossary> | `String` Unique bank code at PayU per payment option. For more info, refer to [Bank and Card Codes for Integration](https://docs.payu.in/docs/bank-and-card-codes-for-integration). | CC |
| offer_key `mandatory for Offers` | `String` Offer key identifier to apply a specific offer. Use Fetch Offer API to fetch the list. | flat150Off@03q62aqtF34n |
| offer_auto_apply `mandatory for Offers` | `String` Set to `true` to automatically apply the best available offer. If `false`, only the specified offer_key is applied. | true |
| api_version `mandatory for Offers` | `String` API version of the _payment API. Must be 14 or later for offers. | 14 |
| user_token `mandatory for UPI/NB/Wallet` | `String` Used by the offer engine to apply velocity rules at user level. Format: `merchant_key:unique_user_identifier`. Card Based Offers: If passed, velocity rules apply on this token; if not, on card number. UPI/NB/Wallet: Mandatory — if not passed, validation rules won't apply. | vqpS7W:test123 |
