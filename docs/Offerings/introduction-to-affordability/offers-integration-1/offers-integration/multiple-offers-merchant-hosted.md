...
Make the payment request using the **_payment** API using the following additional parameters for Offers. You need to send an additional parameter (**user token)**, **api_version** as 14, and hash as described in the following table. This user token would be used to identify the customer for applying velocity rules.
...
<Accordion title="Transaction-level offer response" icon="fa-code">
  ```plaintext
  [transaction_offer] => {
    "offer_data": [
      {
...
<Accordion title="SKU-based offer response" icon="fa-code">
  ```plaintext
  [cart_details] => {
    "total_items": "3",
  ...
  | cart\_details.total\_discount | This field contains the total discount amount for the offer. |
  | cart\_details.net\_cart\_amount | This field contains the net cart amount. |
  | sku\_details | This parameter contains an object in a JSON format that has the SKU details. **Note**: The response can have multiple sku\_details object as an offer can have multiple SKUs. |
  ...
  | sku\_details.is\_nce | This field specifies whether the SKU is No-Cost-EMI offer. |
...
If you want to refund the payment to the customer, refer to [Refund Transaction.](https://devguide.payu.in/api/refund-apis-2/cancel_refund_transaction/)
