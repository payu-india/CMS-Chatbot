...
---
---
title: Checkout Details and EMI Integration Steps
excerpt: Integrate Get Checkout Details (NTB Seamless) and Get EMI Checkout Details APIs in sequence for custom checkout and EMI eligibility.
deprecated: false
hidden: false
metadata:
  title: PayU Checkout Details and EMI Integration Steps
  description: Step-by-step integration for Get Checkout Details API (NTB Seamless Journey) and Get EMI Checkout Details API.
  keywords:
    - Get Checkout Details API
    - Get EMI Checkout Details API
    - NTB seamless journey
    - PayU checkout integration
    - EMI eligibility API
  robots: index
---

This page describes a two-step integration using PayU’s **Get Checkout Details (NTB Seamless Journey)** and **Get EMI Checkout Details** APIs. Use these in order when building custom checkout pages that need payment options, EMI details, and customer eligibility.
...
## Step 1: Get Checkout Details – NTB Seamless Journey
...
```json
{
  ...
  "details": {
    "paymentOptions": {
      "emi": { "all": { "dc": { "hasEligible": true, "all": { "UTIB": { "title": "Axis Bank", "tenureOptions": { ... } } } } } },
