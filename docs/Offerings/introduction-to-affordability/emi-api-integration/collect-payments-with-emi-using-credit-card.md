...
| :--- | :--- | :--- |
| key | `String` Merchant key provided by PayU during onboarding. | JP\*\*\*\*g |
| txnid | `String` The transaction ID is a reference number for a specific order that is generated by the merchant. | kjsdho834580 |
| amount | `String` The payment amount for the transaction. | 10.00 |
| productinfo | `String` A brief description of the product. | iPhone |
| firstname | `String` The first name of the customer. | Ashish |
| email | `String` The email address of the customer. | |
| phone | `String` The phone number of the customer. | |
| pg | `String` It defines the payment category. In this integration, "EMI" must be specified. | EMI |
| bankcode | `String` Post this parameter to identify payment options with unique bank codes and use getEmiAmountAccordingToInterest API to check for EMI code for corresponding tenure. For the list of EMI codes, refer to [EMI Codes](doc:emi-codes). | EMI03 |
| ccnum | `String` Use 13-19 digit card number for credit/debit cards (15 digits for AMEX, 13-19 for Maestro) and validate with LUHN algorithm. Refer to [Card Number Formats](https://docs.payu.in/docs/card-number-formats) and display error message on invalid input. | 5123456789012346 |
...
| Parameter | Description | Example |
| :--- | :--- | :--- |
| threeDS2RequestData | `JSON` This parameter must contain the following information in JSON format. For more information, refer to [Handling 3DS Secure 2.0 Transaction](https://docs.payu.in/docs/collect-payments-with-cards-seamless#/handling-guest-checkout-transactions). | |
