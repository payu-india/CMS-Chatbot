...
✅ **Done!** I've removed all the empty columns and retained only the three essential columns: Parameter, Description, and Example. I also fixed the hash row where the formula was split across multiple columns - it's now properly consolidated in the Description column.
</Accordion>
