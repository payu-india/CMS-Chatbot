...
metadata:
  robots: index
---
NEFT (National Electronic Fund Transfer) or RTGS (Real-Time Gross Settlement) facilitates secure fund transfers from one bank account to another on a one-to-one basis. PayU EFTNET offers NEFT/RTGS as an payment mode through non-seamless (PayU Hosted) or seamless integration (Merchant Hosted or S2S). This part of the document provides the steps to integrate the EFTNET (NEFT/RTGS) and its other offerings:
