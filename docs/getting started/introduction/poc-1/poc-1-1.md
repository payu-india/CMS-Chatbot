...
/* Respond to the actual space this page is given (e.g. inside a docs platform's
   content column), not the browser window — fixes squeezing/overlap when embedded
   next to frozen sidebars. */
.page-root{
  container-type:inline-size;
  container-name:page;
  width:100%;
}
/* Grid/flex items default to min-width:auto, which blocks them from shrinking
   below their content's natural size — this is what causes overlap instead of
   a clean stack when space is tight. Force them to actually shrink. */
.hero-grid > *, .layout > *, .flow-stepper > *, .verify-box > *{
  min-width:0;
}
...
@container page (max-width:760px){.hero-grid{grid-template-columns:1fr;}}
...
@container page (max-width:860px){.layout{grid-template-columns:1fr;} .rail{display:none;}}
@media (max-width:860px){.layout{grid-template-columns:1fr;} .rail{display:none;}}
