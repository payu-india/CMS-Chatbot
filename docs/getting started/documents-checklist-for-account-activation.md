---
title: Documents Checklist for Account Activation
excerpt: ''
deprecated: false
hidden: false
metadata:
  title: Documents Checklist for Account Activation
  description: >-
    The document outlines a checklist of required documents for various entities
    such as individuals, sole proprietorships, partnerships, LLPs, and
    companies, among others, for onboarding and compliance purposes, including
    specific requirements based on industry type like educational services,
    e-commerce, and healthcare. It also details additional compliance documents
    needed for risk checks and the necessary declarations for Ultimate
    Beneficiary Owners (UBO) and senior management.
  keywords:
    - Documents Checklist for Account Activation
    - ' Doc Checklist'
    - ' Account Activation Docs'
    - ' Checklist for Acct Activation'
    - ' Doc Checklist'
  robots: index
next:
  description: ''
---
This section provides the document checklist for the following entities:

- [Individual](#individual)
- [Sole Proprietorship](#sole-proprietorship)
- [Partnership](#partnership)
- [LLP](#llp)
- [Private Limited](#private-limited)
- [Public Limited](#public-limited)
- [Trust](#trust)
- [Society](#society)
- [Government](#government)
- [One Person Company](#one-person-company)
- [Hindu Undivided Family](#hindu-undivided-family)
- [Artificial Judicial Person](#artificial-judicial-person)
- [Local Authority](#local-authority)

These are additional documents are required based on the industry type:

- [Educational Services](#educational-services) 
- [E-Commerce](#e-commerce)
- [Donations/Crowdfunding](#donationscrowdfunding)
- [Healthcare](#healthcare)
- [Additional Compliance Documents to facilitate Risk Checks](https://docs.payu.in/docs/documents-checklist-for-account-activation#additional-compliance-documents-to-facilitate-risk-checks)

Additionally, the following details are required as a part of Onboarding compliance:

- [UBO and Senior Management](#ubo-and-senior-management-declarations)
  - [UBO Declaration](#ubo-ultimate-beneficiary-owner-declaration)
  - [Senior Management Details](#senior-management-details)

## Individual

<details>
<summary>1. PAN Card of the Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of the Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

## Sole Proprietorship

<details>
<summary>1. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>2. Two Government Proofs</summary>

Kindly provide any two of the required government proofs to facilitate your onboarding: GSTIN, MSME, IEC, FSSAI, ITR, Shop establishment, trade license, Tax document (CST/VAT), Utility bills such as electricity, water, landline telephone bills

- Ensure that the shared certificate is active at the time of submission.
- The shared certificate should have the same legal and trade name as shared in the onboarding KYC journey.

</details>

<details>
<summary>3. Address Proof of the Signing Authority</summary>

<AddressProof />

</details>
<details> <summary>PAN Card of Signing Authority</summary>

<PANCard />

</details>

## Partnership

- **Note**: Please provision the list of partners/senior members of the organisation during your onboarding.

<details>
<summary>1. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>2. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>3. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>4. Partnership Deed</summary>

- Upload complete partnership deed. All partners have to sign on deed and it should be on stamp paper.
- The document should have the profit sharing ratios.
- In case of any amendments or changes, the supplement or amendment deed should also be uploaded along with the original deed.</details>

<details>
<summary>5. Government Issued Certificate</summary>

<GovtIssuedCertificate />

</details>

<details>
<summary>6. PAN Card of Partnership</summary>

<PANCard />

</details>

<details>
<summary>7. Authorization Letter/BR Letter</summary>

<AuthorizationLetter />

</details>

<details>
<summary>8. Latest Shareholding Pattern</summary>

<ShareholdingPattern />

</details>

## LLP

- **Note**: Please provision the list of partners/senior members of the organisation during your onboarding.

<details>
<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>4. Government Issued Certificate (Certificate of Incorporation)</summary>

<GovtIssuedCertificate />

</details>

<details>
<summary>5. LLP Deed</summary>

- Kindly upload complete LLP deed, it should be verified by notary or registrar on stamp paper.
- The document should have the profit sharing ratios.
- In case of any amendments or changes, the supplement or amendment deed should also be uploaded along with the original deed.</details>

<details>
<summary>6. PAN Card of LLP</summary>

<PANCard />

</details>

<details>
<summary>7. Authorization Letter/Board Resolution Letter</summary>

<AuthorizationLetter />

</details>

<details>
<summary>8. Latest Shareholding Pattern</summary>

<ShareholdingPattern />

</details>

## Private Limited

- **Note**: 
  - Please provision the list of directors/senior members of the organisation during your onboarding.
  - If the entity is registered as NPL as per Sec 8/25, it should be registered on Darpan portal.

<details>
<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>4. Government Issued Certificate (Certificate of Incorporation)</summary>

<GovtIssuedCertificate />

</details>

<details>
<summary>5. Memorandum of Association</summary>

- Upload complete Memorandum of Association having all pages.
- Missing pages will lead to rejection in Memorandum of Association.</details>

<details>
<summary>6. Articles of Association</summary>

- Upload complete Articles of Association having all pages.
- Missing pages will lead to rejection in Articles of Association.</details>

<details>
<summary>7. PAN Card of Company</summary>

- The document will be validated via API. However, in case of any discrepancies, keep this document handy to process your onboarding.</details>

<details>
<summary>8. Authorization Letter</summary> 

- Authorization/BR letter copy should be on company letterhead.
- Authorization/BR letter should be duly signed/stamped and dated by at least one director/Partner. Self attestation or signature must not be done and only other director or partner must attest or sign in this regard.

</details>

<details>
<summary>9. Latest Shareholding Pattern</summary>

<ShareholdingPattern />

</details>

## Public Limited

- **Note**: 
  - Please provision the list of directors/senior members of the organisation during your onboarding.
  - If the entity is registered as NPL as per sec 8/25 then it should be registered on Darpan portal.

<details>
<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details><details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>4. Government Issued Certificate (Certificate of Incorporation)</summary>

<GovtIssuedCertificate />

</details>

<details>
<summary>5. Memorandum of Association</summary>

- Upload complete Memorandum of Association having all pages.
- Missing pages will lead to rejection in Memorandum of Association.</details>

<details>
<summary>6. Articles of Association</summary>

- Upload complete Articles of Association having all pages.
- Missing pages will lead to rejection in Articles of Association.</details>

<details>
<summary>7. PAN Card of Company</summary>

<PANCard />

</details>

<details>
<summary>8. Authorization Letter</summary>

<AuthorizationLetter />

</details>

<details>
<summary>9. Latest Shareholding Pattern</summary>

<ShareholdingPattern />

</details>

## Trust

<details>
<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>4. Government Issued Certificate</summary>

<GovtIssuedCertificate />

</details>

<details>
<summary>5. Trust Deed</summary>

- Kindly upload complete deed copy.
- Trust deed should be either notarized or verified by registrar.</details>

<details>
<summary>6. Beneficiary List</summary>

- Kindly upload the list of trustees/beneficiaries/settlor/members as on date.
- It should be on company letterhead and should be duly signed and stamped by an authorized person.</details>

<details>
<summary>7. PAN Card of Trust</summary>

<PANCard />

</details>

<details>
<summary>8. Authorization Letter</summary>

<AuthorizationLetter />

</details>

<details>
<summary>9. List of Trustees with Shareholding Pattern</summary>

- These documents may be needed if you do not manually disclose your UBO.
- Provisioned shareholding document should clarify 100% distribution of ownership and should be duly signed and stamped by an authorized person with date.
- Along with the shareholding document, kindly share the KYC document (PAN and address proof) of all individuals holding 10% or more ownership.</details>

<details>
<summary>10. Darpan Portal Document</summary>

This document is only required for NPOs or NGOs, where they are Trust entity only.

- Kindly upload a declaration that no individual or entity holds any shareholding or profit in the trust.
- It should be declared by an authorized person of the trust.</details>

## Society

<details>
<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>4. Government Issued Certificate</summary>

<GovtIssuedCertificate />

</details>

<details>
<summary>5. Bye-Laws for Society</summary>

- Kindly upload complete copy of bye laws or Memorandum of Association.
- Missing pages will lead to rejection.</details>

<details>
<summary>6. PAN Card of Society</summary>

<PANCard />

</details>

<details>
<summary>7. Authorization Letter</summary>

<AuthorizationLetter />

</details>

<details>
<summary>8. List of members certified by Registrar of society with shareholding</summary>

- Shareholding document providing 100% ownership and should be duly signed and stamped by an authorized person with date.
- Upload KYC documents (PAN and Address proof) of individuals having more than 15% ownership.</details>

## Government

<details>

<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>4. Authorization Letter</summary>

<AuthorizationLetter />

</details>

<details>
<summary>5. Evidence of registration or Office order</summary>

- Upload Registration Certificate/ UGC Care certificate/GST.</details>

## One Person Company

<details>
<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>4. Government Issued Certificate</summary>

<GovtIssuedCertificate />

</details>

<details>
<summary>5. Memorandum of Association</summary>

- Upload complete Memorandum of Association having all pages.
- Missing pages will lead to rejection in Memorandum of Association.</details>

<details>
<summary>6. Articles of Association</summary>

- Upload complete Articles of Association having all pages.
- Missing pages will lead to rejection in Articles of Association.</details>

<details>
<summary>7. PAN Card of Company</summary>

<PANCard />

</details>

## Hindu Undivided Family

<details>
<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Authorization Letter</summary>

<AuthorizationLetter />

</details>

<details>
<summary>4. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
  <summary>5. Other docs</summary>

- Power of attorney or another suitable authorisation granted to act on its behalf
- Duly signed and stamp by KARTA
- HUF deed (If any)

</details>

## Artificial Judicial Person

<details>
<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>4. Authorization Letter</summary>

<AuthorizationLetter />

</details>

<details>
<summary>5. Evidence of registration or Office order</summary>

- Upload Registration Certificate/ UGC Care certificate/GST.</details>

## Local Authority

<details>
<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>4. Authorization Letter</summary>

<AuthorizationLetter />

</details>

<details>
<summary>5. Evidence of registration or Office order</summary>

- Upload Registration Certificate/ UGC Care certificate/GST.</details>

<br />

<details>
<summary>1. PAN Card of Signing Authority</summary>

<PANCard />

</details>

<details>
<summary>2. Address Proof of Signing Authority</summary>

<AddressProof />

</details>

<details>
<summary>3. Bank Account Proof</summary>

<BankAccountProof />

</details>

<details>
<summary>4. Govt Issued Certificate</summary>

<GovtIssuedCertificate />

</details>

<details>
<summary>5. Authorization Letter</summary>

<AuthorizationLetter />

</details>

<details>
<summary>6. PAN Card of Society</summary>

<PANCard />

</details>

## Educational Services 

[block:parameters]
{
  "data": {
    "h-0": "Category",
    "h-1": "Document requirements",
    "0-0": "Schools, Colleges, Universities",
    "0-1": "**Affiliation Certificate**  \n  \n- Ensure that the Affiliation Certificate is active and valid at the time of submission.\n- The certificate should clearly state the affiliation with the relevant educational board or authority.\n- The name on the certificate should match the legal and trade name provided during the onboarding KYC process.\n- The certificate should be issued by a recognized and authorized educational board or institution."
  },
  "cols": 2,
  "rows": 1,
  "align": [
    "left",
    "left"
  ]
}
[/block]


## E-Commerce 

[block:parameters]
{
  "data": {
    "h-0": "Category",
    "h-1": "Document requirements",
    "0-0": "Groceries, Food Delivery, Restaurants, Nutritional Supplements, Food, Gourmet Food, Etc.",
    "0-1": "**FSSAI Certificate**  \n  \n- Ensure that the FSSAI Certificate is active and valid at the time of submission.\n- The certificate should clearly state the FSSAI registration number and the name of the business.\n- The name on the certificate should match the legal and trade name provided during the onboarding KYC process.\n- The certificate should be issued by the Food Safety and Standards Authority of India (FSSAI).\n- The certificate should cover the specific category of food business being operated (e.g., Groceries, Food Delivery, Restaurants)."
  },
  "cols": 2,
  "rows": 1,
  "align": [
    "left",
    "left"
  ]
}
[/block]


## Donations/Crowdfunding 

[block:parameters]
{
  "data": {
    "h-0": "Category",
    "h-1": "Documents required",
    "0-0": "NGO",
    "0-1": "**Form (80G, 12A, 10AC)**  \n  \n- Ensure that the forms (80G, 12A, 10AC) are active and valid at the time of submission.\n- The forms should clearly state the registration numbers and the name of the NGO.\n- The name on the forms should match the legal and trade name provided during the onboarding KYC process.\n- The forms should be issued by the Income Tax Department of India.\n- The forms should cover the specific activities and purposes of the NGO (e.g., Donations, Crowd Funding)."
  },
  "cols": 2,
  "rows": 1,
  "align": [
    "left",
    "left"
  ]
}
[/block]


## Healthcare 

[block:parameters]
{
  "data": {
    "h-0": "Category",
    "h-1": "Documents required",
    "0-0": "Nutrition and Supplements",
    "0-1": "**FSSAI Certificate**  \n  \n- Ensure that the FSSAI Certificate is active and valid at the time of submission.\n- The certificate should clearly state the FSSAI registration number and the name of the business.\n- The name on the certificate should match the legal and trade name provided during the onboarding KYC process.\n- The certificate should be issued by the Food Safety and Standards Authority of India (FSSAI).\n- The certificate should cover the specific category of food business being operated (e.g., Groceries, Food Delivery, Restaurants)."
  },
  "cols": 2,
  "rows": 1,
  "align": [
    "left",
    "left"
  ]
}
[/block]


## Additional Compliance Documents to facilitate Risk Checks

| Category                  | Approval/Certificate |
| ------------------------- | -------------------- |
| Stock Advisory/Trading    | SEBI Approval        |
| Prescribed Medicine       | Form 20/21B          |
| Ayurvedic products        | Ayush Certificate    |
| Jewelry Gold/Silver       | BIS                  |
| Diamond                   | GIA                  |
| Insurance                 | IRDA                 |
| Real Estate               | RERA                 |
| Bill Payment              | BBPS                 |
| Forex                     | RBI Approval         |
| Mutual Funds Broker       | SEBI Approval        |
| Banking/NBFC              | RBI Approval         |
| Drop Shipping             | Supplier Agreement   |
| Payment Facilitator       | RBI Approval         |
| Internet Service provided | ISP                  |
| Cable TV Operator         | DAS License          |
| Bulk Msgs                 | DLT Certificate      |

## UBO and Senior Management Declarations

The following details may need to be disclosed as a part of your merchant onboarding:

- UBO Declaration
- UBO Type
- Senior Management

Where the customer or the owner of the controlling interest is: 

- An entity listed on a stock exchange in India, or 
- iIt is an entity resident in jurisdictions notified by the Central Government and listed on stock exchanges in such jurisdictions, or
- It is a subsidiary of such listed entities; it is not necessary to identify and verify the identity of any shareholder or beneficial owner of such entities.

### UBO (Ultimate Beneficiary Owner) Declaration

Please declare the information regarding all the stakeholders based on their ownership: 

| Business Entity                                             | Shareholding % |
| ----------------------------------------------------------- | -------------- |
| Companies (Public, Private) & LLP                           | > 10%          |
| Partnership                                                 | > 10%          |
| Trust                                                       | > /=10%        |
| Unincorporated associations/Body of Individuals (societies) | > 15%          |

### UBO(Ultimate Beneficiary Owner) Type

UBO Details may vary based on origin of the owner:

| UBO Type    | Details Required                                                   |
| :---------- | :----------------------------------------------------------------- |
| Indian UBO  | UBO Name, PAN, Nationality - Indian, Ownership %, DOB and Address. |
| Foreign UBO | UBO Name, Nationality, Ownership %, Document of Address Proof      |

### Senior Management Details

Please declare the information regarding the senior stakeholders on within the organization based on your business entity:

| Category        | Details Required                                                                                     |
| --------------- | ---------------------------------------------------------------------------------------------------- |
| Partnership/LLP | 1. Please declare all applicable partners <br> 2. Please declare all KMPs (Key Management Personnel) |
| Public/Private  | 1. Please declare all Directors <br> 2. Please declare all KMPs (Key Management Personnel)           |